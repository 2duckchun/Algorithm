const solution = require('./reduce.js')

describe('test tsreduce', () => {
  it('콜백함수가 4개의 파라미터를 모두 받아 실행하기', () => {
    let callbacks;
    [9].tsReduce((acc, e, i, arr) => {
  ...
  it('초기값 -3, 배열 내 모든 숫자를 합함. 요소가 짝수일 때 +1 추가.', () => {
    ...
    expect([1, 2, 3].tsReduce(cb, -3)).toEqual(4)
  })
  it('배열["one","two","three"]을 객체 { one: "1 of 3", two: "2 of 2", three: "3 of 3" }로 만들기', () => {
    ...
    expect(a.tsReduce(cb, {})).toEqual(b)
  })
  ...
  it('배열의 모든 요소 더하기 with 초기값 입력 안한 상태', () => {
    ...
    const test = [1, 2, 3, 4]
    expect(test.tsReduce(cb)).toEqual(10)
  })
